"""
Xiangqi Level 1 Generator: Basic Movement Rules (Explicit Mode)
...
class Level1Generator:
    """Generate Level 1 test cases - basic movement rules (explicit mode)"""
    ...
    def _generate_case(self, piece_type: str, is_valid: bool, case_num: int) -> Optional[Dict]:
        """Generate a single test case (explicit mode - shows before and after states)"""
        ...
        # Explicit mode: two states - before and after
        return {
            ...
            "target": target,
            "states": [
                {"pieces": {start: piece_symbol}, "squares": []},  # State 1: before
                {"pieces": {target: piece_symbol}, "squares": []}  # State 2: after
            ],
    ...
    def generate_all(self, n_cases: int = 100) -> List[Dict]:
        """Generate all Level 1 test cases (explicit mode)"""
        all_cases = []
        ...
        print(
            f"\n✓ Total generated: {len(all_cases)} Level 1 explicit test cases")
        return all_cases
