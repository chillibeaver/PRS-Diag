"""
Xiangqi Level 4 Generator: Flying General (飞将/将帅对面) Constraint
Tests whether a piece blocking the two Kings can move away.
...
class Level4Generator:
    """Generate Level 4 test cases - flying general constraint"""
                ...
                if sq and sq not in occupied:
                    ...
                    for rr in range(min(r, row) + 1, max(r, row)):
                        if self._coords_to_square(col, rr) in occupied:
                            ...
                            break
                    if clear:
                        if same_col_ok or True:  # Include same column moves
                            moves.append(sq)
    ...
    def _get_pawn_moves(self, start: str, color: str, occupied: Set[str]) -> List[str]:
        ...
        if color == 'red':
            ...
            if fwd and fwd not in occupied:
                moves.append(fwd)
            # Sideways (only after crossing river, row >= 5)
            if row >= 5:
                for dc in [-1, 1]:
            ...
            if fwd and fwd not in occupied:
                moves.append(fwd)
            # Sideways (only after crossing river, row <= 4)
            if row <= 4:
                for dc in [-1, 1]:
                    side = self._coords_to_square(col + dc, row)
                    if side and side not in occupied:
    ...
    def _generate_invalid_case(self, case_num: int) -> Optional[Dict]:
        ...
        The blocker is the ONLY piece between the two Kings
        """
        # Choose a palace column for both kings
        king_col_char = random.choice(self.palace_cols)
        king_col = ord(king_col_char) - ord('a')

        # Place red king (row 0-2) and black king (row 7-9)
        ...
        occupied = set(pieces.keys())

        # Find moves that leave the column (would cause flying general)
        ...
        # Filter to only moves that leave the column
        ...
        return {
            ...
            "question": f"Can the {blocker_type.capitalize()} move from {blocker_sq} to {target}?",
            "expected": "no",
    ...
    def _generate_valid_same_column_case(self, case_num: int) -> Optional[Dict]:
        ...
        blocker_color = random.choice(['red', 'black'])
        blocker_type = 'rook'  # Use rook since it can move along column
        ...
        # Find moves along the same column (between the kings)
        same_col_moves = []
    ...
    def _generate_valid_multiple_blockers_case(self, case_num: int) -> Optional[Dict]:
        ...
        pieces = {
            red_king_sq: 'K',
            black_king_sq: 'k',
            moving_blocker_sq: self._piece_symbol(moving_type, moving_color),
            staying_blocker_sq: self._piece_symbol(staying_type, staying_color),
        }
        occupied = set(pieces.keys())

        # Find moves for moving blocker that leave the column
        if moving_type == 'rook':
        ...
        return {
            ...
            "color": moving_color,
            "states": [{"pieces": pieces, "squares": []}],
            "question": f"Can the {moving_type.capitalize()} move from {moving_blocker_sq} to {target}?",
            "expected": "yes",
    ...
    def _generate_pawn_crossed_river_case(self, case_num: int, is_valid: bool) -> Optional[Dict]:
        ...
        """
        # Place both kings to establish which side is which
        ...
        # Ensure kings are not on same column (to avoid flying general)
        ...
        if is_valid:
            # Pawn has crossed river - can move sideways
            if pawn_color == 'red':
                # Red pawn crossed river: row 5-8 (on black's side)
                pawn_row = random.randint(5, 8)
            else:
                # Black pawn crossed river: row 1-4 (on red's side)
                pawn_row = random.randint(1, 4)
        else:
            # Pawn has NOT crossed river - cannot move sideways
            if pawn_color == 'red':
                # Red pawn before river: row 3-4
                pawn_row = random.randint(3, 4)
            else:
                # Black pawn before river: row 5-6
                pawn_row = random.randint(5, 6)

        # Choose pawn column (avoid edges for sideways movement)
        pawn_col = random.randint(1, 7)
        pawn_sq = self._coords_to_square(pawn_col, pawn_row)

        # Ensure pawn is not on same square as kings
        if pawn_sq in [red_king_sq, black_king_sq]:
            pawn_col = (pawn_col + 1) % 7 + 1
            pawn_sq = self._coords_to_square(pawn_col, pawn_row)

        # Target: sideways move
        ...
        pieces = {
            red_king_sq: 'K',
            black_king_sq: 'k',
            pawn_sq: self._piece_symbol('pawn', pawn_color),
        }
        ...
        if is_valid:
            return {
                ...
                "color": pawn_color,
                "states": [{"pieces": pieces, "squares": []}],
                "question": f"Can the Pawn move from {pawn_sq} to {target_sq}?",
                "expected": "yes",
    ...
    def _generate_valid_not_blocking_case(self, case_num: int) -> Optional[Dict]:
        ...
        """
        # Choose different columns for kings
        ...
        pieces = {
            red_king_sq: 'K',
            black_king_sq: 'k',
            piece_sq: self._piece_symbol(piece_type, piece_color),
        }
        occupied = set(pieces.keys())
        ...
        return {
            ...
            "color": piece_color,
            "states": [{"pieces": pieces, "squares": []}],
            "question": f"Can the {piece_type.capitalize()} move from {piece_sq} to {target}?",
            "expected": "yes",
