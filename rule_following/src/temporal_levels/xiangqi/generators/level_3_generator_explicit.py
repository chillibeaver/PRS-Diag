"""
Xiangqi Level 3 Generator: Capture Rules with Blocking (Explicit Mode)
...
class Level3Generator:
    """Generate Level 3 test cases - capture rules with blocking (explicit mode)"""
    ...
    def _generate_knight_capture_valid(self, case_num: int) -> Optional[Dict]:
        ...
        for _ in range(50):
            ...
            enemy_symbol = self._piece_symbol('pawn', enemy_color)
            # State 1: before capture, State 2: after capture (enemy removed)
            return {
                "case_id": f"L3_knight_valid_{case_num}",
                "type": "capture_rule", "subtype": "knight_capture_valid",
                "piece_type": "knight", "color": color,
                "states": [
                    {"pieces": {start: knight_symbol,
                                target: enemy_symbol}, "squares": []},
                    {"pieces": {target: knight_symbol}, "squares": []}
                ],
    ...
    def _generate_knight_capture_blocked(self, case_num: int) -> Optional[Dict]:
        ...
        for _ in range(50):
            ...
            # State 1: before (with blocker), State 2: after (illegal attempt)
            return {
                "case_id": f"L3_knight_blocked_{case_num}",
                "type": "capture_rule", "subtype": "knight_capture_blocked",
                "piece_type": "knight", "color": color,
                "states": [
                    {"pieces": {start: knight_symbol, target: enemy_symbol,
                                block_sq: blocker_symbol}, "squares": []},
                    {"pieces": {target: knight_symbol,
                                block_sq: blocker_symbol}, "squares": []}
                ],
                "question": "Is this a legal capture according to Xiangqi rules?",
                "expected": "no",
    ...
    def _generate_bishop_capture_valid(self, case_num: int) -> Optional[Dict]:
        ...
        for _ in range(50):
            ...
            return {
                ...
                "states": [
                    {"pieces": {start: bishop_symbol,
                                target: enemy_symbol}, "squares": []},
                    {"pieces": {target: bishop_symbol}, "squares": []}
                ],
                "question": "Is this a legal capture according to Xiangqi rules?",
                "expected": "yes",
    ...
    def _generate_bishop_capture_blocked(self, case_num: int) -> Optional[Dict]:
        ...
        for _ in range(50):
            ...
            return {
                ...
                "piece_type": "bishop", "color": color,
                "states": [
                    {"pieces": {start: bishop_symbol, target: enemy_symbol,
                                block_sq: blocker_symbol}, "squares": []},
                    {"pieces": {target: bishop_symbol,
                                block_sq: blocker_symbol}, "squares": []}
                ],
                "question": "Is this a legal capture according to Xiangqi rules?",
                "expected": "no",
        ...
        for _ in range(50):
            ...
            for dc, dr in directions:
                ...
                if screen_sq and target_sq:
                    ...
                    screen_symbol = self._piece_symbol('pawn', screen_color)
                    enemy_symbol = self._piece_symbol('pawn', enemy_color)
                    return {
                        ...
                        "states": [
                            {"pieces": {start: cannon_symbol, screen_sq: screen_symbol,
                                        target_sq: enemy_symbol}, "squares": []},
                            {"pieces": {target_sq: cannon_symbol,
                                        screen_sq: screen_symbol}, "squares": []}
                        ],
                        "question": "Is this a legal capture according to Xiangqi rules?",
                        "expected": "yes",
            ...
            for dc, dr in directions:
                ...
                if target_sq:
                    ...
                    return {
                        ...
                        "piece_type": "cannon", "color": color,
                        "states": [
                            {"pieces": {start: cannon_symbol,
                                        target_sq: enemy_symbol}, "squares": []},
                            {"pieces": {target_sq: cannon_symbol}, "squares": []}
                        ],
                        "question": "Is this a legal capture according to Xiangqi rules?",
                        "expected": "no",
        ...
        for _ in range(50):
            ...
            for dc, dr in directions:
                ...
                if screen1_sq and screen2_sq and target_sq:
                    cannon_symbol = self._piece_symbol('cannon', color)
                    screen1_symbol = self._piece_symbol('pawn', screen1_color)
                    screen2_symbol = self._piece_symbol('pawn', screen2_color)
                    enemy_symbol = self._piece_symbol('pawn', enemy_color)
                    return {
                        ...
                        "piece_type": "cannon", "color": color,
                        "states": [
                            {"pieces": {start: cannon_symbol, screen1_sq: screen1_symbol,
                                        screen2_sq: screen2_symbol, target_sq: enemy_symbol}, "squares": []},
                            {"pieces": {target_sq: cannon_symbol, screen1_sq: screen1_symbol,
                                        screen2_sq: screen2_symbol}, "squares": []}
                        ],
                        "question": "Is this a legal capture according to Xiangqi rules?",
                        "expected": "no",
    ...
    def _generate_pawn_capture_forward_after_river(self, case_num: int) -> Optional[Dict]:
        ...
        for _ in range(50):
            ...
            pawn_symbol = self._piece_symbol('pawn', color)
            enemy_symbol = self._piece_symbol('pawn', enemy_color)
            king_symbol = self._piece_symbol('king', color)
            ...
            return {
                ...
                "states": [
                    {"pieces": {start: pawn_symbol, target: enemy_symbol,
                                king_sq: king_symbol}, "squares": []},
                    {"pieces": {target: pawn_symbol, king_sq: king_symbol}, "squares": []}
                ],
                "question": "Is this a legal capture according to Xiangqi rules?",
                "expected": "yes",
    ...
    def generate_all(self, n_cases: int = 50, verbose: bool = False) -> List[Dict]:
        """Generate all Level 3 test cases (explicit mode)"""
        all_cases = []
        ...
        generators = [
            ...
            (self._generate_pawn_capture_forward_after_river, n_pawn_per_subtype +
             (1 if n_pawn_remainder > 0 else 0), "pawn_forward_after"),
            (self._generate_pawn_capture_sideways_after_river, n_pawn_per_subtype +
             (1 if n_pawn_remainder > 1 else 0), "pawn_sideways_after"),
            (self._generate_pawn_capture_forward_before_river, n_pawn_per_subtype +
             (1 if n_pawn_remainder > 2 else 0), "pawn_forward_before"),
            (self._generate_pawn_capture_sideways_before_river, n_pawn_per_subtype +
             (1 if n_pawn_remainder > 3 else 0), "pawn_sideways_before"),
            (self._generate_pawn_capture_backward, n_pawn_per_subtype +
             (1 if n_pawn_remainder > 4 else 0), "pawn_backward"),
        ...
        if verbose:
            print(
                f"\n✓ Total generated: {len(all_cases)} Level 3 explicit test cases")
