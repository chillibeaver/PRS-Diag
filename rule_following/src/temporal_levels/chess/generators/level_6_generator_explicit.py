"""
Level 6 Generator: Castling + Temporal History + 3 Check Rules (Explicit Version)
Aligned with Predictive version - tests all 3 check constraints for castling
...
- invalid_in_check: 3 states (was 2)
- invalid_through_check: 3 states (was 2)
- invalid_into_check: 3 states (was 2)
...
class Level6Generator:
    """Generate Level 6 test cases - castling with temporal history + 3 check rules (Explicit version)"""
    ...
    def _generate_valid_case(self, case_num: int) -> Optional[Dict]:
        ...
        State 2: Other pieces move
        State 3: Other pieces move again
        ...
        # All 3 critical squares must not be attacked
        ...
        return {
            ...
            "expected": "yes",
            "reasoning": "King and rook have not moved; all 3 check rules (in/through/into) satisfied"
        }
    ...
    def _generate_check_violation_case(self, case_num: int, violation_type: str) -> Optional[Dict]:
        ...
        # State 3: Attempted castling
        state3 = {
            config['king_end']: config['king_symbol'],
            config['rook_end']: config['rook_symbol'],
            attacker_final: attacker_symbol,
            **extra_pieces
        }
